...
# Rate limit throttle: serial/controlled concurrent calls to avoid Groq 429 RPM/TPM bursts
...
def _extract_retry_after(response: httpx.Response, attempt: int) -> float:
    ...
    # Check for "try again in X.XXs" in response body
    try:
    ...
    # Fallback to exponential backoff with jitter (2.5s, 5.0s, 10.0s, ...)
...
async def _execute_chat_request(url: str, headers: dict, payload: dict, retries: int = 6) -> str:
    ...
    for attempt in range(retries):
        try:
            # If we're retrying multiple times due to rate limits on a heavy model, fall back to the faster 8b model
            ...
            async with httpx.AsyncClient(timeout=60.0) as client:
                ...
                # Strip possible reasoning blocks or markdown code fences
                if "```json" in content:
                ...
                # Ensure we extract the outermost JSON object/array
...
async def call_grok(prompt: str, system_message: str = "You are a professional business consultant.") -> str:
    ...
    async with _LLM_SEMAPHORE:
        # Pacing delay between agent node calls to respect Groq rate limits
